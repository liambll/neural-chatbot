...
def _extract_argmax_and_embed(embedding, output_projection=None,
  ...
  def loop_function(prev, _):
    ...
    #print("prev", prev) #(?, 8002)
    #print("prev_symbol", prev_symbol) #(?, 1)
    #print("emb_prev", emb_prev) #(?, 32)
...
def _extract_beam_search(embedding, beam_size, num_symbols, embedding_size,  output_projection=None,
  ...
  def loop_function(prev, i, log_beam_probs, beam_path, beam_symbols):
    ...
    #print("best_probs", best_probs) #(1, 10)
    ...
    #print("emb_prev_reshape", emb_prev)
...
def rnn_decoder(decoder_inputs, initial_state, cell, loop_function=None,
  ...
  with variable_scope.variable_scope(scope or "rnn_decoder"):
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      #print("output", output) #(?, 8002)
      #print("state", state) #c(?, 32) h(?, 32)
...
def beam_rnn_decoder(decoder_inputs, initial_state, cell, loop_function=None,
  ...
  with variable_scope.variable_scope(scope or "rnn_decoder"):
    ...
    #state_size = int(initial_state[0].c.get_shape().with_rank(2)[1])
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      #output = tf.Print(output, [tf.shape(output)], message="Output Shape")
      #print("output", output) #(10, 8002)
      #print("state", state) #c(10, 32) h(10, 32)
      ...
      if output_projection is not None:    
          ...
          outputs.append(output)
      #print("outputs", len(outputs), outputs) #[20x (10,8002)]
      
  #print("log_beam_probs", len(log_beam_probs), log_beam_probs) #[19 x (10,1)]
  #print("beam_path", len(beam_path), beam_path) #[19 x unknown]
  #print("beam_symbols", len(beam_symbols), beam_symbols) #[19 x(10,)]
   
  #tf.reshape(tf.concat(beam_path, 0),[-1,beam_size]), \
   #                     tf.reshape(tf.concat(beam_symbols, 0),[-1,beam_size])
  return outputs, state
...
def embedding_rnn_decoder(decoder_inputs, initial_state, cell, num_symbols,
  ...
  with variable_scope.variable_scope(scope or "embedding_rnn_decoder"):
    ...
    #print("embedding", embedding) #(8002, 32)
    #print("emb_inp", len(emb_inp), emb_inp) #[20x (?, 32)]
...
def embedding_rnn_seq2seq(encoder_inputs, decoder_inputs, cell,
  ...
  with variable_scope.variable_scope(scope or "embedding_rnn_seq2seq"):
    ...
    #print("Encoder_Input", len(encoder_inputs), encoder_inputs) #[20 x (?,1)]
    #print("Encoder_State", encoder_state) #c(?, 32) h(?,32)
    #print("Decoder_Input", len(decoder_inputs), decoder_inputs) #[20 x (?,1)]
